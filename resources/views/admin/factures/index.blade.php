                                            ...
                                            <table
                                                ...
                                                @isset($factures)
                                                    @foreach($factures as $facture)
                                                        <tr>
                                                            <td>{{$facture->user->name}} {{$facture->user->prenom}}</td>
                                                            <td>{{$facture->vanne->numero_van}}</td>
                                                            <td>{{$facture -> oldComteur}}</td>
                                                            <td>{{$facture -> newComteur}}</td>
                                                            <td>{{$facture->newMontant}}</td>

                                                            <td>
                                                                <div class="btn-group" role="group"
                                                                     aria-label="Basic example">
                                                                    <a href="{{route('admin.edit',['id' => $facture->id])}}"
                                                                       class="btn btn-outline-primary btn-min-width box-shadow-3 mr-1 mb-1">تعديل</a>


                                                                    <a href="{{route('admin.destroy',['id' => $facture->id])}}"
                                                                       class="btn btn-outline-danger btn-min-width box-shadow-3 mr-1 mb-1">حذف</a>

                                                                    <a href="{{route('admin.information',['id' => $facture->id])}}"
